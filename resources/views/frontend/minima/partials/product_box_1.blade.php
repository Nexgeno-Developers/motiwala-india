@php
    $cart_added = [];
@endphp
<div class="aiz-card-box h-auto pt-0 hov-scale-img product_boxex" style="background:#F4F5F5;">
      ...
      <a href="{{ $product_url }}" class="d-block height_250">
        ...
        @if ($product->auction_product == 0)
            ...
            <div class="absolute-top-right aiz-p-hov-icon">
                ...
                <a href="javascript:void(0)" class="d-none hov-svg-white" onclick="addToCompare({{ $product->id }})"
    ...
    <div class="p-2 p-md-3 text-left">
        <!-- Product name -->
        <h3 class="fw-400 fs-13 text-truncate-1 lh-1-4 mb-0 text-left font_16">
            <a href="{{ $product_url }}" class="d-block text_clr_green"
                title="{{ $product->getTranslation('name') }}">{{ $product->getTranslation('name') }}</a>
        </h3>
        <div class="fs-16 d-flex justify-content-start">
            @if ($product->auction_product == 0)
                <!-- Previous price -->
